...
export class InputSchemaService {
  ...
  fetchInputSchema(url: string): Observable<Record<string, unknown>> {
    ...
    return this.http.get<Record<string, unknown>>(url).pipe(
      catchError(error => {
        ...
        return throwError(() => new Error(`Failed to fetch input schema: ${error.message || 'Unknown error'}`));
      })
    );
  }
   ...
   */
  private getDefaultValue(value: unknown): string | number | boolean | undefined {
   ...
   */
  private getOptionsValue(value: unknown): string[] | { value: string | number | boolean; label: string }[] | undefined {
   ...
   */
  parseInputSchema(schema: Record<string, unknown>): Observable<ParsedInputSchema> {
    try {
      ...
      if (Array.isArray(schema)) {
        ...
        parsedSchema.sections = [{
          ...
          description: this.getStringValue(schema.description) || '',
          fields: this.parseProperties(schema.properties as Record<string, unknown>, this.getStringArrayValue(schema.required) || []),
          collapsible: false,
          collapsed: false
        }];
      }
      ...
      return throwError(() => new Error(`Failed to parse input schema: ${error instanceof Error ? error.message : 'Unknown error'}`));
    }
  }
   ...
   */
  private parseSections(sections: Record<string, unknown>[]): InputSchemaSection[] {
    return sections.map(section => ({
      name: this.getStringValue(section.name) || 'unnamed_section',
      title: this.getStringValue(section.title) || this.getStringValue(section.name) || 'Unnamed Section',
      description: this.getStringValue(section.description) || '',
      fields: Array.isArray(section.fields) ? this.parseFields(section.fields as Record<string, unknown>[]) : [],
  ...
  private parseFields(fields: Record<string, unknown>[]): InputSchemaField[] {
    return fields.map(field => ({
      ...
      type: this.mapFieldType(this.getStringValue(field.type) || 'string', this.getStringValue(field.format)),
      label: this.getStringValue(field.label) || this.getStringValue(field.title) || this.getStringValue(field.name),
      ...
      validation: typeof field.validation === 'object' && field.validation !== null ? field.validation as Record<string, unknown> : {},
   ...
   */
  private parseProperties(properties: Record<string, unknown>, required: string[] = []): InputSchemaField[] {
    return Object.keys(properties).map(key => {
      const prop = properties[key] as Record<string, unknown>;
      
      // Create a human-readable label from the key
      const label = this.getStringValue(prop.title) || this.createLabelFromKey(key);
      
      // Get description from various sources
      const description = this.getStringValue(prop.description) || this.getStringValue(prop.errorMessage) || this.getStringValue(prop.help_text) || '';
      
      return {
        name: key,
        type: this.mapFieldType(this.getStringValue(prop.type) || 'string', this.getStringValue(prop.format)),
        label: label,
        ...
        placeholder: Array.isArray(prop.examples) && prop.examples.length > 0 ? this.getStringValue(prop.examples[0]) : this.createPlaceholderFromKey(key, prop),
        help_text: this.getStringValue(prop.help_text) || this.getStringValue(prop.errorMessage) || '',
   ...
   */
  private createPlaceholderFromKey(key: string, prop: Record<string, unknown>): string {
   ...
   */
  private parseNextflowInput(input: Record<string, unknown>[]): InputSchemaSection[] {
    return input.map(section => ({
      name: this.getStringValue(section.name) || 'unnamed_section',
      title: this.getStringValue(section.title) || this.getStringValue(section.name) || 'Input Section',
      description: this.getStringValue(section.description) || this.getStringValue(section.help_text) || '',
      fields: section.properties && typeof section.properties === 'object' && section.properties !== null 
        ? this.parseProperties(section.properties as Record<string, unknown>, this.getStringArrayValue(section.required) || []) 
        : [],
      collapsible: true,
      collapsed: false
    }));
   ...
   */
  private parseBindflowSchema(schema: Record<string, unknown>): InputSchemaSection[] {
    if (this.hasItemsWithProperties(schema)) {
      ...
      const section: InputSchemaSection = {
        ...
        title: this.getStringValue(schema.title) || 'Input Parameters',
        description: this.getStringValue(schema.description) || '',
        fields: this.parseProperties(items.properties as Record<string, unknown>, this.getStringArrayValue(items.required) || []),
        collapsible: false,
        collapsed: false
      };
      return [section];
   ...
   */
  private mapFieldType(type: string, format?: string): InputSchemaField['type'] {
   ...
   */
  validateFieldValue(field: InputSchemaField, value: unknown): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
      ...
      case 'string':
        if (typeof value !== 'string') {
          ...
          if (field.validation?.maxLength && value.length > field.validation.maxLength) {
            errors.push(`${field.label || field.name} must be no more than ${field.validation.maxLength} characters`);
          }
      ...
      case 'number': {
        ...
        if (isNaN(numValue)) {
          ...
          if (field.validation?.max !== undefined && numValue > field.validation.max) {
            errors.push(`${field.label || field.name} must be no more than ${field.validation.max}`);
          }
        }
        break;
      }
    ...
    if (field.options && field.options.length > 0) {
      const validOptions = field.options[0] && typeof field.options[0] === 'object' && 'value' in field.options[0]
        ? (field.options as { value: string | number | boolean; label: string }[]).map(opt => opt.value)
        : field.options as string[];
      
      // Type-safe includes check
      const isValidOption = validOptions.some(option => option === value);
      if (!isValidOption) {
        errors.push(`${field.label || field.name} must be one of: ${validOptions.join(', ')}`);
      }
    }
