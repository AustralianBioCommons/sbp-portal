...
describe('Navbar', () => {
  ...
  describe('navigate', () => {
    ...
    it('should handle navigation error gracefully', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Navigation failed:', 'Navigation error');
      expect(component.isMobileMenuOpen()).toBe(false);
    });
  });
  ...
  describe('shouldShowNavItem', () => {
    ...
    it('should show nav item when auth required and user is authenticated', () => {
      const item = { label: 'Protected Item', path: '/protected', requiresAuth: true };
  ...
  describe('isNavItemActive', () => {
    ...
    it('should return true for path with matching tab query parameter', () => {
      const item = { label: 'Binder Design', path: '/themes?tab=binder-design' };
      
      expect(component.isNavItemActive(item)).toBe(true);
    });

    it('should return false for path with non-matching tab query parameter', () => {
      const item = { label: 'Structure Prediction', path: '/themes?tab=structure-prediction' };
    ...
    it('should return true when at least one child is active', () => {
      const item = {
        ...
        children: [
          ...
          { label: 'Structure prediction', path: '/themes?tab=structure-prediction' }
        ]
      };
